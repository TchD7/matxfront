        ...
        <Box flex="1" overflowY="auto" borderRadius="lg" border="1px solid" borderColor="gray.200" bg="white">
            {!isMobile ? (
                    ...
                    <Tbody>
                        {tickets.map((t) => (
                            <Tr key={t.id} cursor="pointer" _hover={{ bg: 'gray.50' }} onClick={() => onOpenTicket(t.id)}>
                                ...
                                <Td>{t.intervention_type?.name}</Td>

                                <Td>{t.equipment_name || 'N/A'}</Td>
                                <Td>{t.technician_name || ''}</Td>
                                <Td>
                                    <Badge colorScheme={STATUS_COLORS[t.status]}>
                ...
                <VStack spacing={3} p={4} align="stretch">
                    {tickets.map((t) => (
                        <Box key={t.id} p={4} borderRadius="lg" border="1px solid" borderColor="gray.200" onClick={() => onOpenTicket(t.id)}>
                            ...
                            <Text fontWeight="600" mb={1}>{t.intervention_type?.name}</Text>
                            <Text fontWeight="600" mb={1}>{t.equipment_name || 'N/A'}</Text>
                            <Text fontSize="xs" color="gray.500" mb={3}>Tech: {t.technician_name || ''}</Text>
